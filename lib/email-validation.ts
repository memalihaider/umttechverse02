...
/**
 * Checks if domain has MX records (basic deliverability check)
 */
...
export async function validateEmail(email: string): Promise<EmailValidationResult> {
  ...
  if (domain) {
    result.hasMxRecords = await checkMxRecords(domain)
    if (!result.hasMxRecords) {
      result.errors.push('Email domain does not have valid MX records')
    }
  } else {
...
/**
 * Quick email validation for frontend use (format + disposable check only)
 * Use this for client-side validation without DNS lookup
 */
 ...
 * Use this for client-side validation (format + disposable check only)
 */
export function validateEmailForFrontend(email: string): { isValid: boolean; message: string } {
  const result: EmailValidationResult = {
    isValid: false,
    isDisposable: false,
    hasMxRecords: false, // Not checked in frontend
    errors: []
  }
  ...
  // Overall validity (MX records not checked in frontend)
  result.isValid = result.errors.length === 0
